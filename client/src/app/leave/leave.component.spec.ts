...
describe('LeaveService', () => {
  ...
  it('should update leave status and approval date', () => {
    const mockResponse = { status: 'Approved', approvalDate: '2024-12-05' };

    service.updateLeaveStatus('Rakesh', 'Approved', '2024-12-05').subscribe((response) => {
      expect(response.status).toBe('Approved');
      expect(response.approvalDate).toBe('2024-12-05');
    });

    const req = httpMock.expectOne('http://localhost:8080/api/leave/Rakesh/status');
